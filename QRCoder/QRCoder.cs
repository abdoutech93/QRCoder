    ...
    public class QRCodeGenerator
    {
        ...
        private string GetFormatString(ECCLevel level, int maskVersion)
        {
            ...
            while (fStrEcc.Length > 10)
            {
                ...
                for (int i = 0; i < fStrEcc.Length; i++)
                    sb.Append(((int) (Int64.Parse(fStrEcc[i].ToString()) ^ (int)(Int64.Parse(generator[i].ToString())))));
            ...
            for (int i = 0; i < fStr.Length; i++)
                sbMask.Append(((int) (Int64.Parse(fStr[i].ToString()) ^ (int) (Int64.Parse(fStrMask[i].ToString())))));
        ...
        private string GetVersionString(int version)
        {
            ...
            while (vStrEcc.Length > 12)
            {
                ...
                for (int i = 0; i < vStrEcc.Length; i++)
                    sb.Append(((int) (Int64.Parse(vStrEcc[i].ToString()) ^ (int) (Int64.Parse(generator[i].ToString())))));
        ...
        private static class ModulePlacer
        {
            ...
            public static int MaskCode(ref QRCode qrCode, int version, ref List<Rectangle> blockedModules)
            {
                ...
                return (int) (Int64.Parse(patterMethod.Name.Substring(patterMethod.Name.Length - 1, 1)) - 1);
            }
            ...
            var version = capacityTable.Where(
                m => m.Details.Where(
                    y => (y.ErrorCorrectionLevel == eccLevel
                          && y.CapacityDict[encMode] >= (int) (Int64.Parse(length.ToString()))).Count() > 0).Select(x => new
              {
                  version = m.Version,
                  capacity = m.Details.Where(y => y.ErrorCorrectionLevel == eccLevel).Single().CapacityDict[encMode]
              }).Min(x => x.version));
            return version;
        }
        ...
        {
            return (int) (Int64.Parse(binStr, (NumberStyles)2));
        }
        ...
        private string PlainTextToBinaryNumeric(string plainText)
        {
            ...
            {
                var dec = (int) (Int64.Parse(plainText.Substring(0, 3)));
            ...
            {
                var dec = (int) (Int64.Parse(plainText.Substring(0, plainText.Length)));
